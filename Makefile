...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t mauritsvanrees/plone-conference-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t mauritsvanrees/plone-conference-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name plone-conference-backend-acceptance -d mauritsvanrees/plone-conference-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name plone-conference-frontend-acceptance --link plone-conference-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d mauritsvanrees/plone-conference-frontend:acceptance
